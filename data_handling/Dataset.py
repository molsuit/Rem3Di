...
from data_handling.preprocessing import get_mace_descriptors, get_ase_atoms
import json
...
class AtomEmbeddingDataset(BaseAtomicDataset):
    ...
    def extend_max_atoms(self,new_max_atoms):
        ...
        self.embeddings = np.pad(self.embeddings,((0,0),(0,new_max_atoms - self.embeddings.shape[1]),(0,0)))
        self.padding_masks = np.pad(self.padding_masks,((0,0),(0,new_max_atoms - self.padding_masks.shape[1])))



class RegressionAtomEmbeddingDataset(BaseAtomicDataset):

    def __init__(self,embeddings,padding_mask, regression_targets, regression_masks, metadata,smiles_list):
...
class DatasetFactory:
    ...
    def from_disk(cls, directory : str):
        ...
        with open(directory + "/smiles_list", "r") as f:
        ...
        if metadata["dataset_type"] == "Pretraining":
            ...
            return RegressionAtomEmbeddingDataset(embeddings, padding_mask, regression_targets, regression_masks,metadata,smiles_list)
        
        else:
            raise ValueError("Unknown dataset type")

    @classmethod
    def from_smiles(cls, iterator : SmilesIterator, mace_caluclator, metadata, regression_target: Optional[np.array] = None, regression_masks: Optional[np.array] = None):
        ...
        with tqdm(total=N_molecules) as pbar:
            while(i < N_molecules):
                try:
                    ...
                    print(f"Reached StopIteration prematurely. Completed {i} reading molecules.")
                    break 

                try:
                    atoms = get_ase_atoms(smiles)
                    # Sample conformers?
                    descriptor = get_mace_descriptors(atoms,mace_caluclator, BFGS_tol,max_steps=BFGS_max_steps)
        ...
        if dataset_type == "Pretraining" or dataset_type == "Evaluation":
            ...
            return RegressionAtomEmbeddingDataset(embeddings, padding_mask, regression_target,regression_masks, metadata, smiles_list)
    ...
    @classmethod
    def from_smiles_with_hdf5_caching(cls, iterator : SmilesIterator, mace_caluclator, metadata, regression_target: Optional[np.array] = None, regression_masks: Optional[np.array] = None, chunk_size = 500,directory = None):
        ...
        # Create datasets for the embeddings, padding masks, regression targets and regression masks
            embeddings = f.create_dataset("embeddings", 
                                          shape = (metadata["N_molecules"],metadata["max_atoms"],metadata["embedding_size"]), dtype='f')
            padding_masks = f.create_dataset("padding_masks",
                                             shape = (metadata["N_molecules"],metadata["max_atoms"]), dtype=np.bool_)
