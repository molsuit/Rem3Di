...
def chiral_difference_loss(predictions, labels, regression_mask):
    ...
    pred_e1[valid] - pred_e2[valid]
    lab_e1[valid] - lab_e2[valid]
...
class BaseMultitaskLoss(nn.Module):
    ...
    def forward(self, sample: Sample, output: ModelOutput):
        loss_per_task = multitask_masked_loss(
            predictions=output.regression_predictions,
            labels=sample.regression_targets,
            regression_mask=sample.regression_masks,
        )
