...
class _Welford:
    ...
    def __init__(self, dim: int, device: torch.device, dtype: torch.dtype = torch.float64):
...
def summarise_total(
    ...
    *,
    ...
    norms = np.concatenate(total_stats.l2_norms) if total_stats.l2_norms else np.zeros(0)
...
def plot_mean_std_per_dim(report: AnalysisReport):
    ...
    for layer, color in zip(report.layers, colors, strict=True):
        ...
        axes[0].bar(x, layer.mean_per_dim, color=color, width=1.0,
                    label=f"layer {layer.layer_index}")
        axes[1].bar(x, layer.std_per_dim, color=color, width=1.0,
                    label=f"layer {layer.layer_index}")
        cursor += layer.width
...
def plot_per_dim_entropy(report: AnalysisReport, dead_threshold: float):
    ...
    fig, axes = plt.subplots(1, len(report.layers), figsize=(6 * len(report.layers), 4),
                             sharey=True, squeeze=False)
    colors = _layer_color_cycle(len(report.layers))
    for ax, layer, color in zip(axes[0], report.layers, colors, strict=True):
        order = np.argsort(layer.H_norm_per_dim)[::-1]
        ax.bar(np.arange(layer.width), layer.H_norm_per_dim[order], color=color, width=1.0)
        ax.axhline(dead_threshold, color="red", linestyle="--", linewidth=1,
