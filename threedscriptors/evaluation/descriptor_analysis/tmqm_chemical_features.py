...
def _chelate_ring_sizes(
    ...
    for i in range(len(donors)):
        for j in range(i + 1, len(donors)):
            ...
            path = _shortest_backbone_distance(
                adj, a, b, max_path, donor_set - {a, b}
            )
            if path is None:
                continue
            size = path + 2
...
def _classify_geometry(
    ...
    if cn == 5:
        ...
        return "trigonal_bipyramidal" if (beta - alpha) / 60 > 0.5 else "square_pyramidal"
    if cn == 6:
...
def _features_for_atoms(atoms: Atoms) -> MetalEnvironmentFeatures:
    ...
    if metal_idx is not None:
        try:
            ...
            nn_info = nn.get_nn_info(mol, metal_idx)  # ty: ignore[invalid-argument-type]
