...
class DatasetComparison:
    ...
    def _plot_umap_by_cluster(
        ...
        ax.set_ylabel("UMAP 2")
        ax.set_title(
            f"Union BitBIRCH FPs (top-{len(top_cids)} clusters colored)"
        )
    ...
    def analyze_clusters(
        ...
        contingency = np.zeros(
            (n_clusters, len(self._entries)), dtype=np.int64
        )
        ...
        pretrain_idx = [
            i for i, e in enumerate(self._entries) if e.role == "pretrain"
        ]
        eval_idx = [i for i, e in enumerate(self._entries) if e.role == "eval"]
        n_pretrain_total = int(contingency[:, pretrain_idx].sum()) if pretrain_idx else 0
        has_pretrain = (
        ...
        for di in eval_idx:
            ...
            eval_clusters_with_pretrain[name] = float(have_pre.sum()) / float(mask.sum())
    ...
    def nn_tanimoto(self, union: _UnionFps) -> NnTanimotoSummary | None:
        ...
        if not pretrain_loaded or not eval_loaded:
            log.warning(
                "NN Tanimoto skipped: need >=1 pretrain and >=1 eval dataset"
            )
    ...
    def _plot_nn_tanimoto(
        ...
        del bins
        items = [
            (name, vals) for name, vals in per_dataset_values.items() if vals.size
        ]
        if not items:
            return
    ...
    def _plot_scaffold_jaccard_heatmap(
        ...
        self.results.append(
            FigureResult(
                figure=fig, file_name=Path(f"scaffold_jaccard_{kind}.png")
            )
        )
