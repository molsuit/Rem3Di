...
def load_geom_parallel(
    ...
    mol_paths = []
    for smiles, sub_dic in drugs_summ.items():
        pickle_path = os.path.join(geom_dir, "rdkit_folder", sub_dic.get("pickle_path", ""))
        if os.path.isfile(pickle_path):
            mol_paths.append(pickle_path)
    ...
    # 2) dispatch work
    args = [(i, p, boltzmann_weight_threshold, max_atoms) for i, p in enumerate(mol_paths)]
    ...
    for structure_idx, (mol_id, conf_id, can_smi, nums, pos) in enumerate(raw_results[:N_structures]):
    ...
    print(f"Processed {submitted} files; built {len(smiles)} structures from {kept_mols} molecules.")
