...
def parse_qm9_xyz(path: Path):
    ...
    atoms = Atoms(
        symbols=symbols, positions=np.asarray(coords), info={"smiles": smiles}
    )
...
def load_qm9(
    ...
    tasks_to_load=list[QM9PropertyNames] | None,
    shuffle: bool = True,
):
    ...
    for f in tqdm(xyz_files):
        ...
        if any(
            [a.GetFormalCharge() != 0 for a in Chem.MolFromSmiles(smiles).GetAtoms()]
        ):
            continue
