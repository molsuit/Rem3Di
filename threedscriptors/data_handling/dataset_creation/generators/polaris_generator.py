...
def get_polaris_task_configs(dataset_name: POLARIS_DATASET ) -> list[TaskConfig]:
    ...
    for task in target_cols:
        configs.append(TaskConfig(name = task, task_type=TaskType.regression, scope=TaskScope.system))
...
class PolarisGenerator(MoleculeGenerator):
    ...
    def __iter__(self):
        ...
        idx = 0
        smiles, regression_targets, regression_masks = load_polaris_dataset(dataset_name = self.dataset_name, datasplit="Train")
            ...
            if len(batch_smiles) >= self.batch_size:
                ...
                yield InputBatch(
                    ...
                    regression_data=RegressionData(
                        targets_system=batch_regression_targets, mask_system=batch_regression_masks
                    ),
                )
