...
class TopNormDescriptorsTask(_BaseAnalysisTask):
    ...
    def run(self, ctx: DescriptorAnalysisContext) -> list[EvalResult]:
        ...
        for panel, (subset_idx, dataset_idx, atoms) in enumerate(
            zip(
                top_subset_idx.tolist(),
                top_dataset_idx.tolist(),
                top_atoms,
                strict=True,
            )
        ):
...
class HDBSCANClusterTask(_ClusterTaskBase):
    ...
    def run(self, ctx: DescriptorAnalysisContext) -> list[EvalResult]:
        ...
        summary = _HDBSCANClusterSummary(
            ...
            cluster_sizes={
                int(cid): int(np.sum(labels == cid)) for cid in cluster_ids.tolist()
            },
            cluster_n_components=self.cluster_n_components,
            cluster_n_neighbors=self.cluster_n_neighbors,
            cluster_min_dist=self.cluster_min_dist,
...
class ChemiscopeClusterTask(_ClusterTaskBase):
    ...
    def run(self, ctx: DescriptorAnalysisContext) -> list[EvalResult]:
        ...
        properties.update(
            {
                "metal": col([f.metal_symbol for f in feats], "TM center element"),
                "metal_block": col(
                ...
                "n_carbonyl": col([f.n_carbonyl for f in feats], "Terminal CO ligands"),
                "n_donor_H": col([f.n_donor_H for f in feats], "Hydride donors (M-H)"),
                "n_chelate_rings": col(
            ...
            properties.update(
                {
                    ...
                    "n_halogen": col([f.n_halogen for f in feats], "Halogen count"),
                }
            )
...
class ClusterChemicalFingerprintTask(_ClusterTaskBase):
    ...
    def run(self, ctx: DescriptorAnalysisContext) -> list[EvalResult]:
        ...
        return [
            PydanticResult(file_name=ctx.file_name(self.summary_file_name), obj=report)
        ]
...
class DescriptorStructureBenchmarkTask(_ClusterTaskBase):
    ...
    kind: Literal["descriptor_structure_benchmark"] = "descriptor_structure_benchmark"
    ...
    def run(self, ctx: DescriptorAnalysisContext) -> list[EvalResult]:
        ...
        mcs_values = sorted(
            {*self.sweep_min_cluster_sizes, self.canonical_min_cluster_size}
        )
        ...
        for mcs in mcs_values:
            ...
            sweep.append(
                _BenchmarkSweepRow(
                    min_cluster_size=mcs,
                    n_clusters=int(cluster_ids.size),
                    noise_fraction=float(np.sum(labels == -1)) / n_points
                    if n_points
                    else 0.0,
...
def _fingerprint_cluster(
    ...
    return _ClusterFingerprint(
        ...
        mean_coordination_number=(float(np.mean(resolved_cn)) if resolved_cn else -1.0),
...
def _atoms_for_indices(dataset: MoleculeDataset, indices: list[int]) -> list[Atoms]:
