...
from remedi.utils.model_utils import get_mace_calculator_irrep_signature
...
class REM3DIModel(nn.Module):
    ...
    def __init__(
        ...
        if mace_calculator is not None:
            assert (
                get_mace_calculator_irrep_signature(mace_calculator)
                == preprocessor.atomic_preprocessor.config.input_irreps
            )
    ...
    def get_remedi_descriptor(self, atoms: Atoms):
        ...
        descriptors = np.expand_dims(
            self.mace_calculator.get_descriptors(atoms, invariants_only=False), axis=0
        )
        ...
        sample = Sample(
            embeddings=torch.from_numpy(descriptors),
            padding_mask=padding_mask,
            atomic_positions=torch.from_numpy(positions).float(),
        )
