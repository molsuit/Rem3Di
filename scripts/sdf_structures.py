...
from threedscriptors.data_handling.dataset_creation.molecule_generators import (
...
mace = mace_off(model = "/share/snw30/projects/mace_model/MACE-OFF24_medium.model", default_dtype= "float64", device = "cuda", enable_cueq = True, return_raw_model=True)
...
creation_config = DatasetCreationConfig(
    path=Path("/share/snw30/projects/threedscriptor/3DMolecularDescriptors/data/pcqm"),
    N_structures=100000,
)
dataset_config = DatasetConfig(embedding_dim=mace_irreps.dim ,irreps = mace_irreps, atom_chunk=450, molecule_chunk=50)
