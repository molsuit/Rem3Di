...
from threedprints.data_handling.dataset import AtomEmbeddingDataset
from threedprints.model.model import Transformer, TransformerDecoder, TransformerEncoder
from threedprints.model.pretraining import (
    train_loop,
    validation_loop,
)
...
dataset = AtomEmbeddingDataset.reload_from_disk(
    f"{DATA_DIR}/descriptor_data.npy", f"{DATA_DIR}/descriptor_mask.npy"
)
...
for epoch in range(hyperparameter["epochs"]):
    train_loss = train_loop(
        training_loader, model, optimizer, scheduler, hyperparameter, device
    )
