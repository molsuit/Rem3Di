...
from model.pretraining import train_loop, validation_loop, get_warmup_inverse_sqrt_scheduler
from torch.utils.data import random_split, DataLoader
from torch.optim.lr_scheduler import OneCycleLR

from data_handling.Dataset import AtomEmbeddingDataset
...
scheduler = OneCycleLR(optimizer, max_lr=hyperparameter["learning_rate"], total_steps=num_opt_steps)

#scheduler = get_warmup_inverse_sqrt_scheduler(optimizer, warmup_steps=0.1*hyperparameter["total_steps"], d_model=256)

for epoch in range(hyperparameter["epochs"]):
 
    train_loss = train_loop(training_loader,model,optimizer,scheduler, hyperparameter,device)
