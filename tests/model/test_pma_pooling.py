...
def _make_model(
    d_in=32, d_out=64, num_heads=4, head_dim=64, k_seeds=3, dropout=0.0, use_mlp=True
):
...
def test_bad_configuration_raises():
    ...
    with pytest.raises(AssertionError):
        _ = _make_model(
            d_in=16, d_out=63, num_heads=4, head_dim=64, k_seeds=2
        )  # d_out not divisible
    with pytest.raises(AssertionError):
        _ = _make_model(
            d_in=16, d_out=64, num_heads=8, head_dim=30, k_seeds=2
        )  # 30%8!=0
