...
Covers:
  ...
  * MoleculeDataset writes/reads total_charge & multiplicity alongside the
...
def test_xyz_generator_defaults_to_closed_shell_without_keys(tmp_path: Path) -> None:
    ...
    # No spin_key configured -> default to closed-shell multiplicity 1.0
    # (matches MACE's documented default).
    assert batch.multiplicity == [1.0]
...
def test_copy_data_stage_propagates_charge_and_spin() -> None:
    ...
    assert data_batch.multiplicity.dtype == torch.float64
    torch.testing.assert_close(
        data_batch.total_charge, torch.tensor([1.0, -2.0], dtype=torch.float64)
    )
    torch.testing.assert_close(
        data_batch.multiplicity, torch.tensor([0.5, 1.5], dtype=torch.float64)
    )
...
def test_copy_data_stage_defaults_when_input_missing() -> None:
    ...
    torch.testing.assert_close(
        data_batch.multiplicity, torch.zeros(1, dtype=torch.float64)
    )
...
def test_yield_molecules_collate_batches_charge_and_spin() -> None:
    ...
    torch.testing.assert_close(batched.multiplicity, torch.tensor([2.0, 0.5]))
